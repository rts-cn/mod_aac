...
SWITCH_STANDARD_APP(aac_function)
{
	...
	status = switch_core_codec_init(&codec, "AAC", "fmtp", 8000, 20, 1, SWITCH_CODEC_FLAG_ENCODE, NULL, switch_core_session_get_pool(session));

	if (switch_core_codec_init(&L16_codec, "L16", NULL, 8000, 20, 1, SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, NULL, switch_core_session_get_pool(session)) != SWITCH_STATUS_SUCCESS) {
